...
- 🔬 **LLM-as-a-Judge Evaluation**: Comprehensive quality metrics including accuracy, ROUGE, BLEU, and aviation-specific assessments
...
├── llm_judge_evaluator.py    # LLM-as-a-Judge evaluation system
├── evaluation_dashboard.py   # Evaluation results dashboard
├── test_evaluation.py        # Quick evaluation test script
...
The system includes a comprehensive LLM-as-a-Judge evaluation framework:

### Metrics Measured
- **🎯 Accuracy**: Factual correctness of aviation information (LLM-judged)
- **📍 Relevance**: How well answers address specific questions (LLM-judged)  
- **📝 Completeness**: Comprehensiveness of responses (LLM-judged)
- **📄 ROUGE Scores**: Lexical similarity with reference answers (ROUGE-1, ROUGE-2, ROUGE-L)
...
1. **Via Web Interface**: Click the "🔍 Evaluation Dashboard" tab in the main app
2. **Command Line**: Run `python test_evaluation.py` for a quick test
3. **Programmatic**: Import and use `RAGEvaluator` class

### Evaluation Dashboard Features
- 📊 Visual performance metrics (radar charts, bar charts)
- 📈 Performance trends over time
- 📝 Detailed per-question analysis
- 🔍 Individual response comparison with references

### Test Cases
The system includes default aviation test cases covering:
- ETOPS operations
- Flight dispatcher roles  
- MEL usage
- A-CDM processes
